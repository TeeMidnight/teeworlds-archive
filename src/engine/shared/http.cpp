/*
 * This file is from Carbon, a modified version of Teeworlds.
...
#pragma GCC diagnostic pop
	...
	curl_easy_setopt(pH, CURLOPT_USERAGENT,  "TeeworldsArchive 0.7.6 LastDay (" CONF_PLATFORM_STRING "; " CONF_ARCH_STRING ")");
...
void CHttp::RunLoop()
{
	...
	while(true)
	{
		...
		{
			CHttpRequest *pRequest = apNewRequests[0];
			m_pLock->release();
			...
			m_RunningRequests.emplace(pEH, pRequest);
			apNewRequests.remove_index(0);
			continue;
		error_configure:
			curl_easy_cleanup(pEH);
